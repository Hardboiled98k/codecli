...
use super::pinned_npm::{
    prepare_codex_bundle, remove_pinned_bundle_exact, verify_pinned_bundle_receipt,
};
...
const PKG: &str = "@openai/codex";
const COMMAND: &str = "codex";
/// 发货版只安装已审计过 package bin/平台可选包形态的精确版本。
const SUPPORTED_PACKAGE_VERSION: &str = "0.144.5";
const SUPPORTED_VER: (u32, u32, u32) = (0, 144, 5);
/// 过旧则提示用户用原安装器升级；本工具只升级自己拥有的副本。
const MIN_VER: (u32, u32, u32) = (0, 1, 0);
...
fn install_record_path() -> Result<PathBuf, String> {
    super::platform::codecli_state_dir()
        .map(|directory| directory.join("codex-cli.json"))
        .ok_or_else(|| "找不到 Codex CLI 安装记录路径".into())
}
...
pub async fn codex_cli_version() -> Option<String> {
    super::util::spawn_blocking_ok(codex_cli_version_sync)
        .await
        .ok()
        .flatten()
}

pub fn codex_cli_version_sync() -> Option<String> {
...
fn verify_cli_works() -> Result<String, String> {
    ...
    output
        ...
        .ok_or_else(|| "codex --version 无输出".to_string())
}
...
fn unowned_cli_result(version: Option<String>, force: bool) -> Result<InstallResult, String> {
    ...
    if !force && readable_and_current {
        ...
        return Ok(InstallResult {
            ...
            message: format!(
                "Codex CLI 已由其他方式安装 {}，本工具不抢占安装/卸载归属，已跳过",
                version
            ),
        });
    }
    Err(if force {
        "检测到非本工具安装的 Codex CLI；为避免改变你原有安装及 Claude CLI 工作流，本工具拒绝强制升级。请使用原安装器（npm/brew/官方安装器）升级".into()
    } else {
        "检测到非本工具安装的 Codex CLI，但版本过旧或无法验证；本工具不会覆盖并抢占归属。请使用原安装器升级后重试".into()
    })
}
...
#[tauri::command]
pub async fn install_codex_cli(prefer_mirror: Option<bool>) -> Result<InstallResult, String> {
    super::util::spawn_blocking_result(move || {
        with_new_operation(|| install_codex_cli_sync(prefer_mirror))
    })
    .await
}

pub fn install_codex_cli_sync(prefer_mirror: Option<bool>) -> Result<InstallResult, String> {
    install_codex_cli_sync_with_force(prefer_mirror, false)
}

/// `force=true` 只升级本工具在专属 prefix 中拥有的副本。用户自行安装的
/// Codex CLI 即使显式点击升级也不会被覆盖。
pub fn install_codex_cli_sync_with_force(
    ...
    if existing.is_none() {
        ...
        if package_present || launcher_present {
            return Err(
                "CodeCLI 专属 npm prefix 中存在无可信 ownership 的 Codex CLI 残留；为防误认领已拒绝覆盖，请先人工核查状态目录"
                    .into(),
            );
        }
    ...
    let pending = if let Some(record) = existing {
        ...
        if !package_present && !launcher_present && which_cmd(COMMAND).is_some() {
            return Err(
                "ownership 存在但 CodeCLI 专属 Codex CLI 副作用已不存在，同时检测到用户自装 CLI。请先点击卸载以只清理本工具记录，本次拒绝安装/覆盖"
                    .into(),
            );
        }
            ...
            if let Ok(version) = verify_owned_npm_cli(&record.prefix, PKG, COMMAND) {
                if !force && is_supported_version(&version) {
                    ...
                    if ensured_prefix != record.prefix {
                        return Err("Codex CLI ownership prefix 与 PATH 配置目标不一致".into());
                    }
                    return Ok(InstallResult {
                        ok: true,
                        skipped: true,
                        version: Some(version.clone()),
                        message: format!("本工具安装的 Codex CLI 已可用 {}，跳过", version),
                    });
                }
            }
        }
    ...
    check_cancelled()?;
    // 单独调用该 command 也必须守住 Codex 的 Node 18+ 基线，
    // 不只依赖外层安装编排器。
    super::runtime::ensure_node_sync(Some(18))?;
    let ensured_prefix = ensure_owned_npm_prefix()?;
    if ensured_prefix != pending.prefix {
        return Err("Codex CLI Pending ownership prefix 与安装目标不一致".into());
    }
    ...
    if let Some(receipt) = pending.receipt.as_ref() {
        ...
        return Err(
            "Codex CLI Pending 存在副作用但缺少首次发布前 durable 收据；已 fail closed，不执行、不删除、不调用 npm"
                .into(),
        );
    }
    check_cancelled()?;
    let prepared = prepare_codex_bundle(&pending.prefix).map_err(|error| {
        format!("Codex CLI 固定官方 tarball/SRI bundle 准备失败（最终 prefix 尚未发布）: {error}")
    })?;
    ...
    prepared.publish().map_err(|error| {
        format!("Codex CLI 固定 bundle 发布失败（Pending 已持久收据，可精确恢复/卸载）: {error}")
    })?;
    ...
    let version = verify_owned_npm_cli(&receipt_pending.prefix, PKG, COMMAND).map_err(|error| {
        format!(
            "固定 bundle 发布结束但 CodeCLI 专属 Codex CLI 验证失败（Pending ownership 已保留，可重试或卸载）: {error}"
        )
    })?;
    if !is_supported_version(&version) {
        return Err(format!(
            "固定 bundle 的 Codex CLI 版本不是已审计的 {SUPPORTED_PACKAGE_VERSION}（实际 {version}）；Pending ownership 已保留，已拒绝宣告安装成功"
        ));
    }
    ...
    Ok(InstallResult {
        ...
        message: format!(
            "Codex CLI {SUPPORTED_PACKAGE_VERSION} 已从官方固定 tarball 安装到 CodeCLI 专属 prefix，并完成 SHA-512 SRI 与精确归属验证"
        ),
    })
}

#[tauri::command]
pub async fn uninstall_codex_cli() -> Result<InstallResult, String> {
    super::util::spawn_blocking_result(|| with_new_operation(uninstall_codex_cli_sync)).await
}

pub fn uninstall_codex_cli_sync() -> Result<InstallResult, String> {
    let path = install_record_path()?;
    let Some(record) = load_install_record()? else {
        return Ok(InstallResult {
            ...
            skipped: true,
            version: None,
            message: "没有本工具的 Codex CLI 精确归属记录，已跳过；用户自装 CLI 不会被删除".into(),
        });
    };
    ...
    if let Some(receipt) = record.receipt.as_ref() {
        ...
        return Err(
            "Codex CLI ownership 缺少发布前收据且仍有副作用；已拒绝调用 npm 或猜测删除".into(),
        );
    }
    ...
    if package_still_present || launcher_still_present {
        return Err("收据限定的精确卸载后仍有 Codex CLI 包或 launcher；ownership 已保留".into());
    }
    ...
    Ok(InstallResult {
        ...
        message: if other_install_remains {
            "已卸载 CodeCLI 专属 Codex CLI；PATH 中仍有另一份用户安装，未触碰".into()
        } else {
            "已卸载 CodeCLI 专属 Codex CLI，并在确认副作用不存在后删除归属记录".into()
        },
    })
}
...
mod tests {
    ...
    #[test]
    fn only_audited_codex_package_version_is_supported() {
        assert!(is_supported_version("codex-cli 0.144.5"));
        assert!(!is_supported_version("codex-cli 0.144.4"));
        assert!(!is_supported_version("codex-cli 0.144.3"));
    }
